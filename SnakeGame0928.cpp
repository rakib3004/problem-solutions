...
class Snake
{
    private:
        ...
        // time_tail = tail coordinates time = time tail = snakelenght - 1
        char *map;                                  //Map of game
        bool Left, Right, Down, Up, Bool_Condition; //Left = LEFT Right = RIGHT Down = DOWN Up = UP Bool_Condition = JUST Bool_Condition BOOL
    ...
    public:
        Snake()
        {
            map = new char[2000];
            Left = 0;
            Right = 1; //bool 1 = true bool 0 = false
            Down = 0;
            Up = 0;
            Bool_Condition = 0;
            tail = 1;          //Actually it will not have a tail until it eats first apple
            snake_head = 1000; //Position of snake at the beggining
            Dollars = 0;       //Money
            Bonus = 0;
...
void Snake::Start()
{
    ...
    map[snake_head] = char(219); //Let's make head - block
...
void Snake::KEYBOARD()
{
...

    {
        ...
        case 'd':
            ...
            {
                if (Left == 0) //So if a player has clicked 'd' ,then.1:We check if snake is not
                //going left,because how can she turn right,if she's going left?Teleporting?..
                {
                    Left = 0, Up = 0, Down = 0, //Left = false UP = false Down = false
                        Right = 1;              //Right = true !
                }
...
{
    past.insert(past.begin(), snake_head); //Inserting past snake_head position to vector
    KEYBOARD();
    if (Right == 1) //If snake is going right
    {
        snake_head++;
    }
    else if (Left == 1) //If snake is going left
    {
        snake_head--;
    }
    else if (Up == 1) //If snake is going up
    {
        snake_head -= 50;
    }
    ...
    //If it eats an pineapple also Bool_Condition becomes true,that means the
    ...
    else if (map[snake_head] == char(5)) //Same,but maybe it has just eaten bonus apple?
    {
        Bool_Condition = true;
        tail++;
        Pineapple();
    ...
    /**********/
    if (Bool_Condition == false) //If snake has just eaten an apple
    {
        time_tail = past[past.size() - tail]; //Down E Left E T I N G past tail from the map!
        map[time_tail] = ' ';
    }

    Bool_Condition = false; //So the next time line 270 will work again,if apple is no eaten ///
    if (tail != 1)
    {
